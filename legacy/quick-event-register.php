            ...
            */
            ...
            if ( !$verify ) {
                ...
                */
                ...
                $json['success'] = true;
                $values = qem_calc_money_values( $formvalues, array() );
                $formvalues['calculated'] = $values;
                if ( isset( $formvalues['ignore'] ) && $formvalues['ignore'] == 'checked' && qem_get_element( $register, 'ignorepayment' ) == 'checked' ) {
                    ...
                    qem_process_form( $formvalues, true );
                    // @TODO need to pass these values to the function
                    if ( $paypal && $cost ) {
                    ...
                    if ( $paypal && $cost && !qem_get_element( $formvalues, 'ignore' ) ) {
                        if ( $values['amount'] > 0 ) {
                            $json['form'] .= qem_build_paypal_form_esc( $formvalues, $values );
                        }
...
function qem_display_form_unprotected_esc(  $values, $errors, $registered  ) {
    ...
    if ( qem_get_element( $errors, 'spam', false ) ) {
        ...
        document.querySelector("#qem_reload").scrollIntoView();
            ...
            $val = qem_calc_money_values( $values, array() );
            $content_escaped .= qem_build_paypal_form_esc( $values, $val );
        } elseif ( qem_get_element( $register, 'useread_more' ) ) {
        ...
        if ( $cutoffmessage && 'checked' != $cutoff && 'checked' == get_post_meta( $id, 'event_show_cutoff', true ) ) {
                ...
                case 'field5':
                    if ( $useproducts ) {
                        ...
                        for ($i = 0; $i < count( $products ); $i++) {
                            ...
                            $content_product = '<div style="clear:both;"><b><span style="float:left">' . qem_wp_kses_post( $label ) . '</span><span style="float:right;width:3em;">
                       ...
                       data-qem-cost="' . esc_attr( $products[$i]['cost'] ) . '"
                            ...
                            $content_escaped .= apply_filters(
                                'qem_multi_product_line',
                                $content_product,
                                $products[$i],
                                $id,
                                $payment,
...
function qem_verify_form(  &$values, &$errors, $ajax = false  ) {
    ...
    $paypal = get_post_meta( $id, 'event_paypal', true );
    $cost = get_post_meta( $id, 'event_cost', true );
    $event_donation = get_post_meta( $id, 'event_donation', true );
    $payment = qem_get_stored_payment();
    $ic = qem_get_incontext();
